...
function encrypt (db, key, cryptoSettings, nonceOverride) {
	...
	function encryptWithRule (table, entity, rule) {
		...
		if (rule.type === cryptoOptions.BLACKLIST) {
			...
			for (const key in entity) {
				if (key !== table.schema.primKey.name && entity.hasOwnProperty(key) && indices.includes(key) === false && whitelist.includes(key) === false) {
	...
	db.on('ready', function () {
		return db._encryptionSettings.toCollection().last().then((oldSettings) => {
